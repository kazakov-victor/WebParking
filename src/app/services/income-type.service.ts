...
export class IncomeTypeService {
  private incomeTypeUrl = '/incometype';  // URL to web api
  httpOptions = {
  ...
  getIncomeTypes(): Observable<IncomeType[]> {
    return this.http.get<IncomeType[]>(`${environment.BackUrl}${this.incomeTypeUrl}/list`)
      .pipe(
  ...
  getIncomeTypeNo404<Data>(id: number): Observable<IncomeType> {
    const url = `${environment.BackUrl}${this.incomeTypeUrl}/?id=${id}`;
  ...
  getIncomeType(id: number): Observable<IncomeType> {
    const url = `${environment.BackUrl}${this.incomeTypeUrl}/edit/${id}`;
  ...
  searchIncomeTypes(term: string): Observable<IncomeType[]> {
    ...
    return this.http.get<IncomeType[]>(`${environment.BackUrl}${this.incomeTypeUrl}/search?keyword=${term}`).pipe(
      tap(x => x.length ?
  ...
  saveIncomeType(incomeType: IncomeType): Observable<IncomeType> {
    ...
    return this.http.post<IncomeType>(`${environment.BackUrl}${this.incomeTypeUrl}/save`, incomeType, this.httpOptions).pipe(
  ...
  deleteIncomeType(incomeType
    ...
    const url = `${environment.BackUrl}${this.incomeTypeUrl}/delete/${id}`;
