...
export class ChargeService {
  ...
  getCharges(): Observable<Charge[]> {
    return this.http.get<Charge[]>(`${environment.BackUrl}${this.chargeUrl}/list`)
      .pipe(
  ...
  getChargeNo404<Data>(id: number): Observable<Charge> {
    const url = `${environment.BackUrl}${this.chargeUrl}/?id=${id}`;
  ...
  getCharge(id: number): Observable<Charge> {
    const url = `${environment.BackUrl}${this.chargeUrl}/edit/${id}`;
  ...
  searchCharges(term: string): Observable<Charge[]> {
    ...
    return this.http.get<Charge[]>(`${environment.BackUrl}${this.chargeUrl}/search?keyword=${term}`).pipe(
      tap(x => x.length ?
  ...
  countCharge(date: string): Observable<string> {
    ...
    return this.http.post<string>(`${environment.BackUrl}${this.chargeUrl}/count`, date, this.httpOptions).pipe(
  ...
  deleteCharge(charge: Charge | number): Observable<Charge> {
    ...
    const url = `${environment.BackUrl}${this.chargeUrl}/delete/${id}`;
  ...
  updateCharge(charge: Charge): Observable<any> {
    return this.http.put(`${environment.BackUrl}${this.chargeUrl}/edit`, charge, this.httpOptions).pipe(
  ...
  toggleCloseCharge(id: number): Observable<Charge> {
    const url = `${environment.BackUrl}${this.chargeUrl}/toggle/${id}`;
    return this.http.get<Charge>(url).pipe(
