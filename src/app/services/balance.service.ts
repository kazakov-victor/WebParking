...
export class BalanceService {
  private balanceUrl = 'http://localhost:8080/balance';
  httpOptions = {
  ...
  getBalances(): Observable<Balance[]> {
    return this.http.get<Balance[]>(`${this.balanceUrl}/list`)
      .pipe(
  ...
  getBalanceNo404<Data>(id: number): Observable<Balance> {
    const url = `${this.balanceUrl}/?id=${id}`;
  ...
  getBalance(id: number): Observable<Balance> {
    const url = `${this.balanceUrl}/edit/${id}`;
  ...
  searchBalances(term: string): Observable<Balance[]> {
    ...
    return this.http.get<Balance[]>(`${this.balanceUrl}/search?keyword=${term}`).pipe(
      tap(x => x.length ?
